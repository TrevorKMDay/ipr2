...
\newcommand{\cdicov}{$17\,342$}

\newcommand{\cdiagepcg}{$20.6$}
\newcommand{\cdiagescg}{$18.7$}
\newcommand{\cdisexpcg}{$-37.5$}
\newcommand{\cdisexscg}{$-37.5$}
\newcommand{\cdibopcg}{$59.1$}
\newcommand{\cdiboscg}{$38.9$}
\newcommand{\cdivrspcg}{$-19.3$}
\newcommand{\cdivrsscg}{$-23.5$}
...
\newcommand{\bapqpcg}{$-26.7$}
\newcommand{\bapqscg}{$-2.7$}
...
\begin{document}
    ...
    \begin{tikzpicture}
        ...
        \node (cdi1) [outcome, fill=red!30] {CDI\textsubscript{T} PCG};
        \node (cdi2)
            [outcome, fill=blue!30, below of=cdi1, node distance=2cm]
        ...
        \node (vrs) [child, above of=bo, node distance=1.5cm] {Mean VRS} ;
        ...
        \draw [->] (age) to
            [out=180,in=180]
            node[anchor=west] {-0.49}
            (vrs) ;
        \draw [->] (sex) to
            [out=180,in=180]
            node[anchor=east] {1.08}
            (vrs) ;
...
%
%        % bapq effects
        \draw [arrow,thick] (p1bapq) -- node[anchor=south] {\bapqpcg} (cdi1) ;
%
        \draw [->,dashed] (p2bapq)
